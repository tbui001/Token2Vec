...
def main():
    ...
    w2v_pretrain = args["embed"] if args["test"] else print("Please input your embedding model")
    ...
    w2v_model = Word2Vec.load(w2v_pretrain)

    x_train = train_data.iloc[:, 1].apply(lambda text: [w2v_model.wv[word] for word in text.split() if word in w2v_model.wv])
    x_test = test_data.iloc[:, 1].apply(lambda text: [w2v_model.wv[word] for word in text.split() if word in w2v_model.wv])
    ...
    outputFileName = 'XGB_W2V' + file_tag
    DIR_ASSETS = out_loc + '/proposed_model/'
